...
Самыми затратными по времени фукнцией является ```HashCrc32```, она будет оптимизирована в первую очередь.

### HashCrc32 optimization

Вообще говоря, хеш-функция ```Crc32``` была подобрана неслучайно. Дело в том, что существует ассемблерная функция, которая считает этот хеш:
...
Благодаря профилировщику можно искать узкие места в программе, которые можно оптимизировать. Таким образом, точечными низкоуровневыми оптимизациями программа была ускорена в 2,17 раз. Это показывает, что использование только флагов оптимизаций недостаточно.

При этом всегда стоит помнить, что большое число ассемблерных оптимизаций вредит переносимости кода и его читаемости. Поэтому посчитаем коэффициент [Деда](https://github.com/ded32):

DED_coeff = $\frac{\text{ускорение программы}}{количество  ассемблерных  строк} * 1000 = \frac{2170}{11} = 197,27$
